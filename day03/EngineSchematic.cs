...
public class EngineSchematic(string[] gameData)
{
    ...
    public List<Tuple<int, int>> GetGearsPairs()
    {
        ...
        foreach (var s in _symbols)
        {
            ...
            if ((numbers.Count != 0) & (numbers.Count == 2))
                _gearsPairs.Add(new Tuple<int, int>(numbers.Select(e => e.Value).ToArray()[0],
                    numbers.Select(e => e.Value).ToArray()[1]));
        }
    ...
    {
        var sum = 0;
        foreach (var kp in _gearsPairs) sum += kp.Item1 * kp.Item2;
